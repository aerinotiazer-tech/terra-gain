    ...
    <div className="flex flex-col min-h-screen bg-zinc-950 text-zinc-100 px-4 pt-10 pb-[100px] max-w-md mx-auto">
      ...
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {["25k", "50k", "100k", "250k", "500k", "1M+"].map((v) => (
              <button
                ...
                className={getBtnClass(profile.budget === v)}
              >
                {v}
              </button>
            ))}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col justify-between">
            ...
            <div className="flex flex-col gap-2">
              {["Pluie", "Sèche", "Toute"].map((v) => (
                <button
                  key={v}
                  onClick={() => update("saison", v)}
                  className={getBtnClass(profile.saison === v)}
                >
                  {v}
                </button>
              ))}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col justify-between">
            <label className="block text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-3">
              Eau
            </label>
            <div className="flex flex-col gap-2">
              {["Pluvial", "Irrigué", "Proche"].map((v) => (
                <button
                  key={v}
                  onClick={() => update("eau", v)}
                  className={getBtnClass(profile.eau === v)}
                >
                  {v}
                </button>
              ))}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 mt-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button
                key={lg.v}
                onClick={() => setLanguage(lg.v)}
                className={getBtnClass(state.language === lg.v)}
              >
                {lg.l}
              </button>
            ))}
      ...
      <button
        ...
        className="mt-8 w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all"
      >
        {t("btn_view_recs")}
      </button>
    </div>
