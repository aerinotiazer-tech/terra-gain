...
export function Recommendations() {
  ...
  const scoredCrops = useMemo(() => {
    return CROPS.map((c) => {
      let score = 0;
      // Zone climatique 30%
      if (c.zones_climatiques_adaptees.includes(region?.zone_climatique || ""))
        score += 0.3;
      // Budget 20%
    ...
    <div className="flex flex-col min-h-screen bg-zinc-950 text-zinc-100 px-4 pt-10 pb-[100px] max-w-md mx-auto">
      ...
      <div className="space-y-4">
        {scoredCrops.map((c) => (
          <div
            key={c.id}
            className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 relative overflow-hidden group"
          >
            <div className="absolute top-0 right-0 p-4">
              ...
              >
                {(state.compareCropIds || []).includes(c.id)
                  ? "Sélectionné"
                  : "Comparer"}
              </button>
            </div>
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex gap-2 text-xs mt-1">
                  <span
                    className={`px-2 py-0.5 rounded font-bold uppercase ${c.risque_meteo === "Bas" ? "bg-emerald-500/10 text-emerald-400" : c.risque_meteo === "Moyen" ? "bg-amber-500/10 text-amber-400" : "bg-red-500/10 text-red-400"}`}
                  >
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-zinc-800/40 rounded-xl p-3">
                ...
                <span className="font-semibold text-lg text-indigo-300">
                  {(c.rendement_moyen_t_ha * ha).toFixed(1)} t
                </span>
              </div>
              <div className="bg-zinc-800/40 rounded-xl p-3">
                <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest block mb-1">
            ...
            <button
              ...
              className="w-full flex items-center justify-between text-zinc-100 font-bold bg-zinc-800 hover:bg-zinc-700 py-4 px-5 rounded-xl transition-colors"
            >
      ...
      {state.compareCropIds.length === 2 && (
        <div className="fixed bottom-6 left-0 w-full px-4 flex justify-center z-50">
          <button
            ...
            className="bg-amber-500 text-zinc-950 font-black px-6 py-4 rounded-full shadow-[0_10px_25px_rgba(245,158,11,0.5)] flex items-center gap-2"
          >
      ...
      {state.compareCropIds.length !== 2 && (
        <div className="fixed bottom-0 left-0 w-full px-4 pb-6 pt-10 bg-gradient-to-t from-zinc-950 via-zinc-950 to-transparent flex justify-center z-40 gap-4">
          <button
            ...
            className="bg-zinc-800 border border-zinc-700 flex-1 px-4 py-4 rounded-xl font-bold flex flex-col items-center gap-1"
          >
